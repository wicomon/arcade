// Motor de Asteroids portado de references/starter-games/02-asteroids/game.js
// Sin acceso al DOM global: recibe el canvas y reporta estado vía callbacks.
...
class Ship implements Entity {
  ...
  update(dt: number) {
    ...
    if (left) this.angle -= ROT * dt;
    if (right) this.angle += ROT * dt;
...
export function createGame(
  ...
  function loop(ts: number) {
    ...
    if (!paused) {
      ...
      update(dt);
      reportState();
      if (!wasGameOver && state === "gameover") callbacks.onGameOver(score);
      draw();
    }
